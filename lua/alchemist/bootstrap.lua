...
--- Get debug command arguments if debug mode is enabled
---@return table|nil
function M.get_debug_command_args()
	local debug_mod = require("alchemist.debug")
	if debug_mod.is_enabled() then
		return debug_mod.build_debug_command()
	end
	return nil
end
...
function M.prompt_uv_download(callback)
	vim.ui.select({ "Yes, download uv", "No, I'll install it myself" }, {
...
function M.spawn_daemon(callback)
	...
	local job_id = vim.fn.jobstart(cmd, {
		detach = true,
		-- Prevent python stdout/stderr stream buffering
			...
			if not M.has_uv() then
				M.prompt_uv_download(function(approved)
					if not approved then
						callback(
							"UV_BOOTSTRAP_FAILED: uv is required. " .. "Install from https://github.com/astral-sh/uv"
						)
						return
					end
...
function M._do_spawn_and_connect(callback)
	-- Bypass spawning entirely if launching manually in an external terminal
	if vim.g.alchemist_external_daemon then
		M.wait_for_socket(60, 500, function(available)
			if available then
				M._connect_and_initialize(callback)
			else
				callback("DAEMON_UNAVAILABLE: Timed out waiting for external daemon socket")
			end
		end)
		return
	end
	...
	if not M.acquire_lock() then
		vim.notify("[Alchemist] Another instance is starting the daemon, waiting...", vim.log.levels.INFO)
	...
	M.check_socket(function(alive)
		...
		vim.schedule(function()
			M.spawn_daemon(function(err)
				-- RELEASE LOCK HERE: Allows Python process to acquire its own lock file cleanly
				M.release_lock()
				...
				-- Extended retries if debugpy is waiting for DAP client connection
				local debug_mod = require("alchemist.debug")
				local max_retries = (debug_mod.is_enabled() and debug_mod.should_wait_for_client()) and 120 or 20
				M.wait_for_socket(max_retries, 250, function(available)
					if available then
						M._connect_and_initialize(callback)
					else
		...
		vim.schedule(function()
			...
			methods.initialize(function(init_err, result)
				...
				if result and result.status == "no_keys_configured" then
					...
					vim.schedule(function()
						vim.notify(
							"[Alchemist] No API keys configured. " .. "Run :AlchemistSetup to add a provider key.",
							vim.log.levels.WARN
						)
					end)
				end
...
function M._register_all_handlers()
	...
	methods.register_handlers({
		...
		["daemon/key_swapped"] = function(params)
			state.update({
				last_status = vim.tbl_extend("force", state.get().last_status or {}, {
		...
		["daemon/exhausted"] = function()
			...
			vim.schedule(function()
				vim.notify(
					"[Alchemist] All API keys exhausted. "
